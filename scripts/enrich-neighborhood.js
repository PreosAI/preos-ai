...
// ── Single combined fetch: scores + named POIs ────────────
async function fetchAll(prop) {
  const { lat, lng } = prop;

  // Single query fetching everything needed for both
  // scores AND place tab panels, with coordinates
  const query = `[out:json][timeout:45];
(
  node["shop"](around:500,${lat},${lng});
  node["amenity"~"restaurant|cafe|fast_food|bar|pub|supermarket|convenience|pharmacy|bank|post_office|bakery|butcher|greengrocer|marketplace"](around:500,${lat},${lng});
...
out body qt;`;
  ...
  for (const mirror of MIRRORS) {
    try {
      ...
      // ── Count categories for scores ────────────────────
      const c = { walk:0, transit:0, bike:0, noise:0, wellness:0, green:0 };

      // ── Collect named POIs for tab panels ───────────────
      const schools   = [];
      const places    = [];
      const transport = [];
      ...
      for (const el of elements) {
        ...
        // --- Scores counting (same logic as before) ---
        if (t.shop) c.walk++;
...
async function main() {
  ...
  for (const prop of PROPERTIES) {
    ...
    const result = await fetchAll(prop);
    if (!result) {
    ...
    const { counts: c, pois } = result;
    console.log(`  counts: walk=${c.walk} transit=${c.transit} bike=${c.bike} noise=${c.noise} wellness=${c.wellness} green=${c.green}`);
    console.log(`  POIs: schools=${pois.schools.length} places=${pois.places.length} transport=${pois.transport.length}`);
    ...
    console.log(`  💾 saved: walk=${scores.walkability} transit=${scores.transit} bike=${scores.bike} noise=${scores.noise} wellness=${scores.wellness} green=${scores.green}`);
